...
export default function MediaDownload() {
  ...
  const getMediaIcon = () => {
    if (!tool) return <File className="w-16 h-16 text-zinc-600" />;
    const url = tool.link_url?.toLowerCase() || '';
    if (url.match(/\.(zip|rar|7z|tar)$/)) return <FileArchive className="w-20 h-20 text-amber-400" />;
    if (url.match(/\.(mp3|wav|ogg|m4a)$/)) return <Music2 className="w-20 h-20 text-pink-400" />;
    if (url.match(/\.(mp4|webm|mov|avi)$/)) return <Video className="w-20 h-20 text-blue-400" />;
    if (url.match(/\.(pdf|doc|docx|txt|epub)$/)) return <FileText className="w-20 h-20 text-emerald-400" />;
    return <FileCode className="w-20 h-20 text-purple-400" />;
  };
              ...
              <div className="flex flex-col items-center flex-1">
                  ...
                  <div className="relative group mb-8">
                      ...
                      {tool.image_url && tool.image_url.match(/\.(jpeg|jpg|gif|png|webp|svg)$/i) ? (
                         ...
                         <div className="relative w-40 h-40 sm:w-48 sm:h-48 rounded-3xl border border-white/10 shadow-2xl flex items-center justify-center bg-white/5 overflow-hidden">
